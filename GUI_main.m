...
% Last Modified by GUIDE v2.5 08-Feb-2016 13:25:37
    ...
    init = [a* handles.init.CHHb * 100,...
     ...
     ref = [a* handles.ref.CHHb * 100,...
      ...
      Labels = {'a*C_{HHb}b', 'a*C_{OHb}b', 'a*C_{H2O}b', 'a*C_{Lipid}b','StOv', 'b'};
       ...
       hold on
       plot([0:2],handles.ref.StOv*ones(size([0:2])),'--g');
       
       plot([0:2],handles.init.StOv*ones(size([0:2])),'--r');
       ylim([ymin ,ymax])
       set(gca,'ytick',[ymin:0.05:ymax]);
      bar(handles.rec.StOv, 'BaseValue', ymin);
     ...
     %%% plot oxy for the bulk
       ...
       ymax = 1;
       ymin = 0.3;
       plot([0:2],handles.ref.StOb*ones(size([0:2])),'--g');
       hold on
       plot([0:2],handles.init.StOb*ones(size([0:2])),'--r');
       ylim([ymin ,ymax])
       set(gca,'ytick',[ymin:0.05:ymax]);
       bar(handles.rec.StOb, 'BaseValue', ymin);
...
'MenuBar','none',...
'Name','Toolbox for simulation of photoacoustic imaging',...
'NumberTitle','off',...
...
'Parent',h10,...
'Units','pixels',...
'Position', r.*  [50 530 100 20],...
'String','simulation model',...
'Style','text',...
'Tag','text22',...
'CreateFcn', {@local_CreateFcn, blanks(0), appdata} );
...
'Units','pixels',...
'CData',[],...
'Position', r.*  [50 500  250 20],...
'String','Reconstruction Resul: StO for blood vessel and bulk',...
'Style','text',...
'UserData',[],...
'Tag','text24',...
'CreateFcn', {@local_CreateFcn, blanks(0), appdata} );
...
'Parent',h63,...
'Units','pixels',...
'Position', r.*  [15.5 172.5 100.5 80],...
